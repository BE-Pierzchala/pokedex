...
class Data:
    ...
    def open_save(self):
        try:
            with open(DATA_PATH / 'tmp.pickle', 'rb') as file:
                data = pickle.load(file)
                type(data)
                self.typesDict = data.typesDict
                self.multipliers = data.multipliers
                self.pokeType = data.pokeType
            print('Loaded old pokedex!')
        except FileNotFoundError:
            raise FileNotFoundError('File not found! If you want to start a new save add -n or --new_save flag')

    def save(self):
        # TODO remove tmp
        with open(DATA_PATH / 'tmp.pickle', 'wb') as file:
    ...
    def getMultipliers(self):
        ...
        # Plot heatmap
        ax = sns.heatmap(self.multipliers, cmap=cm, linewidth=0.5, xticklabels=self.typesDict.keys(),
                         yticklabels=self.typesDict.keys())
        ax.xaxis.tick_top()
            ...
            if self.getMultiplier(move[1:], type_) == -1:
                ...
                while usr not in ['y', 'n']:
                    usr = input('Conflict! ' + str(move[1:]) + ' : ' + str(type_) + ' \nOld: ' + str(
                        self.getMultiplier(move[1:], type_)) + ' New: ' + str(
                        self.inv_effects[move[0]]) + '\n Replace? y/n\n>')
                    if usr == 'y':
                        self.setMultiplier(move[1:], type_, self.inv_effects[move[0]])
            else:
                print(str(move[1:]) + ' : ' + str(type_) + ' \nOld: ' + str(
                    self.getMultiplier(move[1:], type_)) + ' New: ' + str(self.inv_effects[move[0]]))
                print('Already known!')
                return
